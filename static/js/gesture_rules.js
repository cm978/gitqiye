(function attachGestureRules(root) {
  ...
  const PINCH_CLICK_THRESHOLD = 0.78;
  const SWIPE_OPEN_PALM_THRESHOLD = 0.58;
  ...
  function classifyDemoGesture(handSignal, state, now) {
    ...
    let gesture = null;
    const swipeReady = handSignal.openness >= SWIPE_OPEN_PALM_THRESHOLD && handSignal.palmFacingCamera === true;
    if (center) {
    ...
    if (!gesture && handSignal.pinch >= PINCH_CLICK_THRESHOLD && handSignal.openness >= 0.25) gesture = "click";
    if (!gesture && !swipeReady && handSignal.openness >= 0.72) gesture = "zoom_in";
